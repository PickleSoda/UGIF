...
const Home = () => {
  ...
  useEffect(() => {
    const defaultCategory = categories.find(
      category =>
        category.category === (selectedSegment === 'GIF' ? 'gif' : 'video') &&
        category.name === 'All',
    );
      ...
      <IonHeader mode="ios" className="container">
        ...
        <IonToolbar className="custom-toolbar p-2 mt-2">
          ...
          <IonSegment
            scrollable={true}
            ref={segmentRef}
            value={selectedSegmentCategory}
            className="scroll-hide"
            onTouchStart={onTouchStart}
            onTouchMove={onTouchMove}
            onTouchEnd={onTouchEnd}
            onIonChange={ev =>
              setSelectedSegmentCategory(ev.detail.value?.toString() ?? '')
            }
          >
            {categories
              .filter(
